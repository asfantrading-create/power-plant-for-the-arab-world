import { t, L, errorMessage } from '../i18n.js';
import { api } from '../api.js';
import { state } from '../state.js';
import { h, icon, copyText, toast } from '../ui.js';
import { licenseSummary } from './activation.js';
import { BRAND, contactLinks } from '../lib/brand.js';
...
export async function render(container, params, ctx) {
  ...
  container.append(h('div', { class: 'hero-banner mb' }, h('div', { class: 'flex gap-lg' }, h('img', { src: 'assets/logo.png', width: 72, height: 72, alt: '' }), h('div', null, h('h1', null, t('app.name')), h('p', { class: 'muted' }, `${t('app.short')} · ${t('about.version')} ${b.version}`))),
    ...
    h('div', { class: 'card mt' }, h('h3', null, t('about.company')), h('div', { class: 'company-card' }, h('img', { src: BRAND.logo, alt: '' }), h('div', { class: 'grow' }, h('b', null, t('brand.company')), h('p', { class: 'muted small' }, t('about.companyText')), h('div', { class: 'flex wrap' }, ...contactLinks())),
      h('div', { class: 'flex col' }, h('span', { class: 'small muted' }, t('about.manual')), h('button', { class: 'btn sm', onClick: async () => { try { await api('app:openManual', { lang: 'ar' }); } catch (err) { toast(errorMessage(err.code), 'error'); } } }, icon('file'), t('about.manualAr')), h('button', { class: 'btn sm', onClick: async () => { try { await api('app:openManual', { lang: 'en' }); } catch (err) { toast(errorMessage(err.code), 'error'); } } }, icon('file'), t('about.manualEn'))))));
}
