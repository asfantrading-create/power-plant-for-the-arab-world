  ...
  chapters: [
    ...
    { title: 'License activation', sections: [
      { title: 'The activation screen', body: [
        ...
        { ul: ['**Subscription plan**: a monthly or annual subscription (or a fixed expiry date). The program shows the expiry date and the days left, a reminder at sign-in thirty days before expiry, and a bar at the top of the screen two weeks before with the Asfan Co. contact links for renewal.', '**Seats**: the maximum number of active accounts of all roles (0 means unlimited).', '**Included modules**: digital twin and/or exams. A module that is not included disappears from the menus.', '**Included technologies**: plants of technologies not included show a lock badge; their data can be browsed, but the digital twin cannot be started and they are excluded from generated questions.', '**Machine lock**: optional; a machine-locked license works only on the matching computer.'] },
      ] },
      { title: 'Activation error messages', body: [
        { table: [['The key format is invalid', 'Make sure the whole key was copied without missing lines or spaces, or use the .lic file.'], ['Invalid signature', 'The key is forged or was issued for another build. Ask the vendor for a new key.'], ['The subscription has expired', 'The subscription period is over. Contact Asfan Co. to renew, then paste the renewal key on this screen (or under Administration → License while the subscription is still active). Your data and accounts are not affected.'], ['This key is locked to another computer', 'Send this computer’s ID to the vendor to obtain a key for it.'], ['System clock rollback detected', 'The computer date was moved backwards. Set the correct date and time and restart the program.'], ['The key belongs to another product', 'The key is not for Arab Power Twin.']], head: ['Message', 'Meaning and remedy'] },
      ] },
    ] },
    ...
    { title: 'System: license, settings and data', sections: [
      { title: 'License', body: [
        ...
        'Shows the license state and details: licensee, organisation, subscription plan (monthly, annual or fixed term), issue and expiry dates with days left, seats and accounts in use, included modules and technologies, license ID and the machine ID with a copy button.',
        { ul: ['**Renewing the subscription**: two weeks before expiry a bar appears at the top of the program with the Asfan Co. contact links (and a reminder at sign-in thirty days before). Request the renewal key and activate it on this page. If the subscription expires before renewal, the activation screen appears at start-up and entering the new key brings back all data and accounts unchanged.', '**Activate a new key**: when renewing or upgrading, paste the new key or load the .lic file and click Activate; the new license replaces the old one without losing any data.', '**Remove license**: returns the program to the activation screen (super admin only).'] },
      ] },
      { title: 'Settings', body: [
