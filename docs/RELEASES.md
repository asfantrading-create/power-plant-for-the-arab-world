...
> ملاحظة: `electron-updater` يحتاج أن يكون المستودع **عاماً** (Public) ليقرأ `latest.yml` بدون رمز وصول. إذا أردت إبقاء الكود خاصاً: أنشئ مستودعاً عاماً منفصلاً للإصدارات فقط وغيّر `publish.owner/repo` في `electron-builder.yml`، أو استخدم خادم HTTPS خاصاً بك (ارفع `latest.yml` و`ArabPowerTwin-Setup.exe` و`.blockmap` إلى مجلد واحد) واضبط «رابط خادم التحديثات» في إعدادات البرنامج أو `publish: {provider: generic, url: ...}` قبل البناء.
