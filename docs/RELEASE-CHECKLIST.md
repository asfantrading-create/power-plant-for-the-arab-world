...
2. أصدر له ترخيصاً من جهازك **بصفحة مولّد التراخيص** (بلا أوامر): افتح `tools\license-generator\index.html` بالنقر المزدوج، اختر ملف المفتاح الخاص `tools\license-cli\keys\vendor-private.pem`، أدخل بيانات العميل، النوع (مدى الحياة أو تاريخ انتهاء)، عدد المقاعد (0 = غير محدود)، معرّف الجهاز إن أردت ربطه بجهاز واحد (يقرأه العميل من شاشة التفعيل `APT-XXXX-...`)، والوحدات والتقنيات المشمولة، ثم «توليد مفتاح الترخيص» و«تنزيل ملف .lic». الشرح الكامل في [LICENSING.md](LICENSING.md).
   البديل بالأوامر:
```powershell
# مدى الحياة
npm run license -- issue --name "د. أحمد" --org "جامعة X" --email a@x.edu --seats 40 --out "جامعة-X.lic"
# محدد المدة (ينتهي في التاريخ المحدد)
npm run license -- issue --org "شركة Y" --type term --expires 2027-09-30 --seats 10 --out "Y.lic"
